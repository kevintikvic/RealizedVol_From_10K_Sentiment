...
# Play around… Sentences --------------------------------------------------
# 
# install.packages("NLP")
# install.packages("openNLP")
# install.packages("qdap")

library(NLP); library(openNLP); library(qdap)


testdoc <- "This is sentence number one; And this is sentence number two. Let's see how R handles these: this is sentence number 3 or 4, or 3.4."
sent_detect(testdoc)

sent_token_annotator <- Maxent_Sent_Token_Annotator()
sentSplit(as.data.frame(testdoc))

AAA_sl <- tm_map(AAA, sent_detect)
inspect(AAA_sl)